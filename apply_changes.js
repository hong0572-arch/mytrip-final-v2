...
`;
    ...
    if (content.includes(injectionHook) && !content.includes('flatPlaces')) {
        content = content.replace(injectionHook, injectionHook + '\\n' + newVars);
    }

    // 2. Add marker listener
    const markerTarget = '                        const marker = new google.maps.Marker({ position: place.coordinates, map, icon: { path: google.maps.SymbolPath.CIRCLE, fillColor: dayColor, fillOpacity: 1, strokeColor: "white", strokeWeight: 2, scale: 12 }, label: { text: (placeIdx + 1).toString(), color: "white", fontWeight: "bold", fontSize: "12px" }, zIndex: 100 + index });\\n                        markersRef.current.push(marker);';
    const markerReplacement = \`                        const marker = new google.maps.Marker({ position: place.coordinates, map, icon: { path: google.maps.SymbolPath.CIRCLE, fillColor: dayColor, fillOpacity: 1, strokeColor: "white", strokeWeight: 2, scale: 12 }, label: { text: (placeIdx + 1).toString(), color: "white", fontWeight: "bold", fontSize: "12px" }, zIndex: 100 + index });
    ...
    if (content.indexOf('marker.addListener') === -1) {
        content = content.replace('markersRef.current.push(marker);', markerReplacement.split('\\n')[1] + '\\n' + markerReplacement.split('\\n')[2] + '\\n' + markerReplacement.split('\\n')[3] + '\\n' + markerReplacement.split('\\n')[4] + '\\n' + markerReplacement.split('\\n')[5] + '\\n' + markerReplacement.split('\\n')[6] + '\\n' + markerReplacement.split('\\n')[7] + '\\n' + markerReplacement.split('\\n')[8]);
    }

    // 3. Replace return block entirely
    ...
    const endStr = '            {/* PDF 변환용 숨겨진 A4 서식 유지 */}';
                                ...
                                {infoModalTab === 'hotels' && (
                                    <div className="space-y-4">
                                        {hotels.length > 0 ? hotels.map((hotel, idx) => (
                                            <div key={idx} className="place-card bg-white p-4 rounded-2xl border border-gray-200 shadow-sm relative group cursor-pointer hover:border-indigo-500 transition-all" onClick={() => { const link = getKlookLink(\`\${hotel.name} \${userInfo?.destination || ""}\`, '695932', language); window.open(link, '_blank'); }}>

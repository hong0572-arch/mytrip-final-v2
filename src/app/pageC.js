...
export default function Home() {
  ...
  const generatePlan = async () => {
    ...
    setLoading(true);
    try {
      console.log("Sending request to /api/generate..."); // 디버깅 로그
      ...
      console.log("API Response Data Received:", data); // 서버에서 받은 원본 데이터 확인

      if (data.result) {
        console.log("Setting Valid Result:", data.result);
        setResult(data.result);
      } else {
        console.error("No result field in response:", data);
        alert("오류: " + (data.error || "여행 일정을 생성하지 못했습니다. (서버 응답 없음)"));
      }
    } catch (error) {
      console.error("Fetch Error:", error);
      alert("서버 통신 중 오류가 발생했습니다. 개발자 도구의 콘솔을 확인해주세요.");
    } finally {
      setLoading(false);
    }
  };

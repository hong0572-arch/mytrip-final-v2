...
export default function TripCoach({ itineraries = [], userData = {}, onShowToast, language = 'ko', onTabChange }) {
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-16">
      {/* 헤더 */}
      <header className="flex justify-between items-center px-4 pt-12 pb-4 sticky top-0 z-40 bg-gradient-to-b from-gray-900 to-transparent backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-black text-white tracking-tight">Trip Coach</h1>
        </div>
        <div className="bg-white/10 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-white/10 flex items-center gap-1.5 shadow-sm text-xs font-bold text-gray-300">
      ...
      <div className="px-4 pt-4 space-y-6">
        ...
        {itineraries.length === 0 ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Target Itinerary</span>
        ...
        {selectedTrip && (
          <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white/10 p-4 rounded-2xl border border-white/10">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-initial bg-indigo-600/20 text-indigo-400 hover:bg-indigo-600/30 font-bold text-xs px-3 py-2 rounded-xl transition"
                    >
                      + 250ml 💧
                    </button>
                    <button
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white/10 p-4 rounded-2xl border border-white/10">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-initial bg-rose-600/20 text-rose-400 hover:bg-rose-600/30 font-bold text-xs px-3 py-2 rounded-xl transition"
                    >
                      + 1,000보 🏃
                    </button>
                    <button
            ...
            <div className="space-y-4">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Today's Coaching Highlights</span>
              
              {/* 상황에 맞는 카드 렌더링 */}
              {/* [출발 전 - 준비 단계 코칭 카드] */}
              {currentPhase === 'prep' && (
                <div className="bg-gradient-to-br from-indigo-900/60 to-purple-900/60 border border-indigo-500/20 rounded-[28px] p-5 shadow-lg relative overflow-hidden">
                  <div className="flex gap-4">
                    ...
                    <div className="overflow-hidden">
                      ...
                      <p className="text-xs text-brand-secondary font-bold mt-2 flex items-center gap-1">
              ...
              {/* [수분 부족 코칭 카드] */}
              {hydrationScore < 60 && (
                <div className="bg-gradient-to-br from-blue-950/60 to-cyan-950/60 border border-blue-500/20 rounded-[28px] p-5 shadow-lg relative overflow-hidden">
                  <div className="flex gap-4">
                    <div className="w-12 h-12 bg-blue-500/20 rounded-2xl flex items-center justify-center text-blue-400 shrink-0 border border-blue-500/30 animate-pulse">
                      <Droplet size={22} />
                    </div>
                    <div className="overflow-hidden">
                      <h4 className="text-white font-black text-base">탈수 위험 알림! 물 섭취 필요 💧</h4>
                      <p className="text-xs text-gray-300 leading-relaxed mt-1.5">
                        활동량에 비해 오늘의 물 섭취량이 부족합니다. 탈수를 예방하고 컨디션을 좋게 유지하기 위해 최소 1L의 물을 더 드시길 권장합니다.
                      </p>
                      <button
                        onClick={() => handleAddWater(500)}
                        className="mt-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-4 py-2 rounded-xl transition"
                      >
                        지금 물 500ml 원샷 🥤
                      </button>
                    </div>
              ...
              {(fatigueLevel === 'tired' || fatigueLevel === 'exhausted') && (
                <div className="bg-gradient-to-br from-rose-950/60 to-orange-950/60 border border-rose-500/20 rounded-[28px] p-5 shadow-lg relative overflow-hidden">
                  <div className="flex gap-4">
                    <div className="w-12 h-12 bg-rose-500/20 rounded-2xl flex items-center justify-center text-rose-400 shrink-0 border border-rose-500/30">
              ...
              {/* [치안 및 안전 주의 카드] */}
              {currentPhase === 'during' && (
                <div className="bg-gradient-to-br from-emerald-950/60 to-teal-950/60 border border-emerald-500/20 rounded-[28px] p-5 shadow-lg relative overflow-hidden">
                  <div className="flex gap-4">
                    ...
                    <div className="overflow-hidden">
                      <h4 className="text-white font-black text-base">안심 가이드 및 꿀팁 🛡️</h4>
                      <p className="text-xs text-gray-300 leading-relaxed mt-1.5">
                        {meta.tip}
                      </p>
                      <div className="mt-3 flex items-center gap-2">
                        <span className="text-[10px] bg-emerald-900 text-emerald-400 px-2 py-0.5 rounded-full font-bold">안전지침</span>
                        <span className="text-[10px] text-gray-400">항상 에코백과 바지 주머니 관리를 철저히!</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div key={opt.key} className="flex justify-between items-center py-2 border-b border-white/10">
                    ...
                    <button
                      ...
                      className={`w-11 h-6 rounded-full transition-all relative ${
                        notificationConfig[opt.key] ? 'bg-brand-primary' : 'bg-gray-700'
                      }`}
                    >
              ...
              <div className="bg-brand-primary/10 border border-brand-primary/20 rounded-2xl p-4">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="bg-gray-800 text-white text-xs font-bold px-3 rounded-xl border border-white/10 outline-none"
                  >
